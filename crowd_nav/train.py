...
def main(args):
    ...
    window_size = train_config.trainer.batch_size
    # configure trainer and explorer
    memory = ReplayMemory(capacity, window_size, device)
    model = policy.get_model()
    target_model = policy.get_model()
    batch_size = train_config.trainer.batch_size
    optimizer = train_config.trainer.optimizer
    if policy_config.name == 'model_predictive_rl':
        trainer = MPRLTrainer(model, target_model, policy.state_predictor, memory, device, policy, writer, batch_size, optimizer, env.human_num,
    ...
    if args.resume:
        ...
        explorer.run_k_episodes(500, 'train', update_memory=True, imitation_learning=True)
        trainer.optimize_epoch(il_epochs)
        policy.save_model(il_weight_file)
        logging.info('Finish imitation learning. Weights saved.')
        logging.info('Experience set size: %d/%d', len(memory), memory.capacity)
        '''
    trainer.update_target_model(model, os.path.join(args.output_dir, 'temp.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='configs/mp_separate.py')
