...
from HRO_sim.envs.utils.state import tensor_to_joint_state
from HRO_sim.envs.utils.utils import point_to_segment_dist
...
from crowd_nav.policy.graph_model_hetero import PyHGTModel_V, PyHGTModel_S
...
class ModelPredictiveRL(Policy):
    ...
    def configure(self, config):
        ...
        if self.linear_state_predictor:
            ...
            if self.share_graph_model:
                ...
                graph_model1 = PyHGTModel_V(
                    config=config,
                    conv_name=config.hgt.conv_name,
                    use_RTE=config.hgt.use_RTE,
                    use_time=config.use_time,
                    n_hid=config.hgt.n_hid,
                    output_size=256,
                    human_state_dim=config.HRORNN.human_state_dim,
                    robot_state_dim=config.HRORNN.robot_state_dim
                )
                self.value_estimator = ValueEstimator(config, graph_model1)
                graph_model2 = PyHGTModel_S(
                    config=config,
                    conv_name=config.hgt.conv_name,
                    use_RTE=config.hgt.use_RTE,
                    use_time=config.use_time,
                ...
                self.model = [graph_model1, graph_model2, self.value_estimator.value_network,
                              self.state_predictor.human_motion_predictor, self.state_predictor.map_predictor]
        ...
        if self.state_predictor.trainable:
            if self.share_graph_model:
                ...
                return {
                    ...
                    'value_network': self.value_estimator.value_network.state_dict(),
                    'motion_predictor': self.state_predictor.human_motion_predictor.state_dict(),
                    'map_predictor': self.state_predictor.map_predictor.state_dict()
                }
        else:
            return {
                    'graph_model': self.value_estimator.graph_model.state_dict(),
    ...
    def load_state_dict(self, state_dict):
        if self.state_predictor.trainable:
            ...
            self.state_predictor.map_predictor.load_state_dict(state_dict['map_predictor'])
        else:
            self.value_estimator.graph_model.load_state_dict(state_dict['graph_model'])
            self.value_estimator.value_network.load_state_dict(state_dict['value_network'])
    ...
    def predict(self, state, masks, rnn_hxs, local_map = None):
        """
        ...
        if self.device == torch.device('cuda:0'):
            ...
            masks = masks.cuda()
            rnn_hxs['local_map_rnn_v'] = rnn_hxs['local_map_rnn_v'].cuda()
            rnn_hxs['agent_node_rnn_v'] = rnn_hxs['agent_node_rnn_v'].cuda()
            rnn_hxs['robot_node_rnn_v'] = rnn_hxs['robot_node_rnn_v'].cuda()
            rnn_hxs['final_node_rnn_v'] = rnn_hxs['final_node_rnn_v'].cuda()
            rnn_hxs['local_map_rnn_s'] = rnn_hxs['local_map_rnn_s'].cuda()
            rnn_hxs['agent_node_rnn_s'] = rnn_hxs['agent_node_rnn_s'].cuda()
            rnn_hxs['robot_node_rnn_s'] = rnn_hxs['robot_node_rnn_s'].cuda()
        elif self.device is not None:
            local_map = local_map.to(self.device)
            masks = masks.to(self.device)
            rnn_hxs['local_map_rnn_v'] = rnn_hxs['local_map_rnn_v'].to(self.device)
            rnn_hxs['agent_node_rnn_v'] = rnn_hxs['agent_node_rnn_v'].to(self.device)
            rnn_hxs['robot_node_rnn_v'] = rnn_hxs['robot_node_rnn_v'].to(self.device)
            rnn_hxs['final_node_rnn_v'] = rnn_hxs['final_node_rnn_v'].to(self.device)
            rnn_hxs['local_map_rnn_s'] = rnn_hxs['local_map_rnn_s'].to(self.device)
            rnn_hxs['agent_node_rnn_s'] = rnn_hxs['agent_node_rnn_s'].to(self.device)
            rnn_hxs['robot_node_rnn_s'] = rnn_hxs['robot_node_rnn_s'].to(self.device)
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in action_space_clipped:
                #time1 = time.clock()
                state_tensor = state.to_tensor(add_batch_size=True, device=self.device)
                next_state, rnn_hxs_s = self.state_predictor(local_map, masks, rnn_hxs, state_tensor, action)
                next_state[2][:,0:23,...] = (next_state[2][:,0:23,...] == next_state[2][:,0:23,...].max(dim=1, keepdim=True)[0]).to(dtype=torch.float32)
                max_next_return, max_next_traj, rnn_hxs_v = self.V_planning(next_state, masks, rnn_hxs, self.planning_depth, self.planning_width)
                reward_est = self.estimate_reward(state, local_map, action)
                #reward_est = self.estimate_reward((next_state[0],next_state[1]), next_state[2], action)
                value = reward_est + self.get_normalized_gamma() * max_next_return
                self.action_values.append(value.detach().cpu().numpy())
                #self.action_values.append(value)
                if value > max_value:
                    max_value = value
                    max_action = action
                    max_rnn_hxs = dict(rnn_hxs_v, **rnn_hxs_s)
    ...
    def V_planning(self, state, masks, rnn_hxs, depth, width):
        ...
        current_state_value, rnn_hxs_v = self.value_estimator(state, masks, rnn_hxs)
        if depth == 1:
            return current_state_value, [(state, None, None)], rnn_hxs_v
    ...
    def estimate_reward(self, state, local_map, action):
        ...
        local_occ = local_map[0, 1:23, ...].sum(axis=0)
        ...
        for i, human in enumerate(human_states):
            ...
            closest_dist = point_to_segment_dist(px, py, ex, ey, 0, 0) * self.env.resolution - human.radius - robot_state.radius
            if closest_dist < 0:
                collision = True
                break
        ...
        if self.kinematics == 'holonomic':
            rx = 64 + action.vx * self.time_step
            ry = 64 + action.vy * self.time_step
        else:
            theta = robot_state.theta + action.r
            rx = 64 + np.cos(theta) * action.v * self.time_step
            ry = 64 + np.sin(theta) * action.v * self.time_step
        collision_with_obstacle = False
        samplePoint = self.env.getRandomPointInCircle(100, robot_state.radius / self.env.resolution, rx, ry)
        for i in range(len(samplePoint)):
            rx, ry = samplePoint[i][0], samplePoint[i][1]
            if local_occ[ry][rx] >= 1:
        ...
        #reaching_goal = norm(end_position - np.array([robot_state.gx, robot_state.gy])) * self.env.resolution < robot_state.radius

        if self.env.objectnav:
            reaching_goal = self.env.success_checker_for_object_nav(end_position, self.env.success_range)
        else:
            reaching_goal = norm(end_position - np.array([robot_state.gx, robot_state.gy])) * self.env.resolution < robot_state.radius

        if collision:
            reward = self.env.collision_with_human_penalty
        elif collision_with_obstacle:
            reward = self.env.collision_with_obstacle_penalty
        elif reaching_goal:
            reward = self.env.success_reward
        elif dmin < self.env.discomfort_dist:
            # adjust the reward based on FPS
            reward = (dmin - self.env.discomfort_dist) * self.env.discomfort_penalty_factor * self.time_step
        else:
            #reward = 0
            #potential_cur = - norm(end_position - np.array(self.env.robot.get_goal_position())) * self.env.resolution
            potential_cur = - self.env.fmm_dists[self.env.object_goal_idx, int(end_position[1]), int(end_position[0])] * self.env.resolution
            reward = 0.2 * (- abs(potential_cur) - self.env.potential)
