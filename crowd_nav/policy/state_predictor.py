...
from crowd_nav.policy.graph_model_hetero import MapConv_up
...
class StatePredictor(nn.Module):
    def __init__(self, config, graph_model, time_step):
        ...
        self.human_motion_predictor = mlp(config.hgt.X_dim_s, config.hgt.motion_predictor_dims)
        self.map_predictor = MapConv_up(num_channel = 23)
        self.time_step = time_step

    def forward(self, local_map, masks, rnn_hxs, state, action, detach=False):
        ...
        state_embedding, rnn_hxs = self.graph_model(state, masks, rnn_hxs, local_map)
        ...
        next_human_states = self.human_motion_predictor(state_embedding[:, 1:21, :])

        map_state = rearrange(state_embedding[:, 21:, :], 'B L M -> B M L')
        B, L, M = map_state.size()
        map_state = map_state.reshape(B, L, int(M**0.5), int(M**0.5))
        next_map_state = self.map_predictor(map_state)
        next_map_state = torch.cat([next_map_state, local_map[:,23:,...]], dim=1)
        next_observation = [next_robot_state, next_human_states, next_map_state]
        return next_observation, rnn_hxs
