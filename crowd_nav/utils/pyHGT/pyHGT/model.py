...
class GNN(nn.Module):
    def __init__(self, in_dim, n_hid, num_types, num_relations, n_heads, n_layers, dropout = 0.2, conv_name = 'hgt', prev_norm = False, last_norm = False, use_RTE = True, adapt_in_dim = True):
        ...
        self.adapt_in_dim = adapt_in_dim 
        ...
        if adapt_in_dim:
            ...
            assert n_hid == in_dim, "if adapt_in_dim is False, n_hid should equal to in_dim, to ensure gnn takes correct input"
    ...
    def forward(self, node_feature, node_type, edge_time, edge_index, edge_type):
        ...
        for t_id in range(self.num_types):
            ...
            if idx.sum() == 0:
                continue
            if self.adapt_in_dim:
                res[idx] = torch.tanh(self.adapt_ws[t_id](node_feature[idx]))
            else: 
                res[idx] = torch.tanh(node_feature[idx])
