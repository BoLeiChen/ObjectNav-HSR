...
class MPRLTrainer(object):
    def __init__(self, value_estimator, target_model, state_predictor, memory, device, policy, writer, batch_size, optimizer_str, human_num,
    ...
    def update_target_model(self, target_model, file):
        #self.target_model = copy.deepcopy(target_model)
        torch.save(target_model.state_dict(), file)
    ...
    def optimize_epoch(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for data in self.data_loader:
                robot_states, human_states, local_map, actions, values, _, _, next_human_states, next_local_map\
                                                                                , mask, recurrent_hidden_state = data

                # optimize value estimator
                self.v_optimizer.zero_grad()
                outputs = self.value_estimator((robot_states, human_states, local_map))
                ...
                if self.state_predictor.trainable:
                    ...
                    if update_state_predictor:
                        ...
                        _, next_human_states_est, next_map_states_est = self.state_predictor(local_map, (robot_states, human_states), None)
                        loss = self.criterion(next_human_states_est, next_human_states) + self.criterion(next_map_states_est, next_local_map)
                        loss.backward()
            ...
            logging.info('Average loss in epoch %d: %.6f, %.6f', epoch, epoch_v_loss / len(self.memory),
    ...
    def optimize_batch(self, num_batches, episode):
        ...
        for data in self.data_loader:
            robot_states, human_states, local_map, actions, _, rewards, next_robot_states, next_human_states, \
                                                                    next_local_map, mask, recurrent_hidden_state = data

            robot_states = rearrange(robot_states, 'B L N F -> (L B) N F')
            human_states = rearrange(human_states, 'B L N F -> (L B) N F')
            local_map = rearrange(local_map, 'B L C N F -> (L B) C N F')
            rewards = rearrange(rewards, 'B L F -> (L B) F')
            next_robot_states = rearrange(next_robot_states, 'B L N F -> (L B) N F')
            next_human_states = rearrange(next_human_states, 'B L N F -> (L B) N F')
            next_local_map = rearrange(next_local_map, 'B L C N F -> (L B) C N F')
            mask = rearrange(mask, 'B L F -> (L B) F')
            for k, v in recurrent_hidden_state.items():
                recurrent_hidden_state[k] = rearrange(v, 'B L N F -> (L B) N F')

            # optimize value estimator
            self.v_optimizer.zero_grad()
            recurrent_hidden_state_1 = {}
            N = robot_states.size(0)
            for k, v in recurrent_hidden_state.items():
                recurrent_hidden_state_temp = torch.Tensor().to(self.device)
                for i in range(0, N, self.batch_size):
                    recurrent_hidden_state_temp = torch.cat([recurrent_hidden_state_temp, v[i:i+1, ...].to(self.device)], dim=0)
                recurrent_hidden_state_1[k] = recurrent_hidden_state_temp
            outputs, _ = self.value_estimator((robot_states, human_states, local_map), mask, recurrent_hidden_state_1)

            gamma_bar = pow(self.gamma, self.time_step * self.v_pref)

            recurrent_hidden_state_2 = {}
            for k,v in recurrent_hidden_state.items():
                recurrent_hidden_state_temp = torch.Tensor().to(self.device)
                for i in range(0, N, self.batch_size):
                    recurrent_hidden_state_temp = torch.cat([recurrent_hidden_state_temp, v[i+1:i+2, ...].to(self.device)], dim=0)
                recurrent_hidden_state_2[k] = recurrent_hidden_state_temp
            tragets, _ = self.target_model((next_robot_states, next_human_states, next_local_map), mask, recurrent_hidden_state_2)
            target_values = rewards + gamma_bar * tragets
            ...
            if self.state_predictor.trainable:
                ...
                if update_state_predictor:
                    self.s_optimizer.zero_grad()
                    (_, next_human_states_est, next_map_states_est), _ = self.state_predictor(local_map, mask,
                                        recurrent_hidden_state_1, (robot_states, human_states), None, detach=self.detach_state_predictor)

                    next_map_states_est[:, 0:23, ...] = (
                                next_map_states_est[:, 0:23, ...] == next_map_states_est[:, 0:23, ...].max(dim=1, keepdim=True)[
                            0]).to(dtype=torch.float32)

                    loss = self.criterion(next_human_states_est, next_human_states) + \
                            self.criterion(next_map_states_est[:, 0:23, ...], next_local_map[:, 0:23, ...])
                    loss.backward()
                    self.s_optimizer.step()
        ...
        logging.info('Average loss : %.6f, %.6f', average_v_loss, average_s_loss)
